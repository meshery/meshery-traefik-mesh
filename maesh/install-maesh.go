...
var (
	localFile                  = path.Join(os.TempDir(), "maesh.tar.gz")
	destinationFolder          = path.Join(os.TempDir(), "maesh")
	basePath                   = path.Join(destinationFolder, "%s")
	installFile                = path.Join(basePath, "install/kubernetes/maesh.yaml")
	installWithmTLSFile        = path.Join(basePath, "install/kubernetes/maesh-demo-auth.yaml")
...
func (iClient *MaeshClient) downloadFile(localFile string) error {
	...
	resp, err := http.Get(iClient.maeshReleaseDownloadURL)
	if err != nil {
		err = errors.Wrapf(err, "unable to download the file from URL: %s", iClient.maeshReleaseDownloadURL)
		logrus.Error(err)
		return err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		err = fmt.Errorf("unable to download the file from URL: %s, status: %s", iClient.maeshReleaseDownloadURL, resp.Status)
		logrus.Error(err)
		return err
	}
...
func (iClient *MaeshClient) downloadMaesh() (string, error) {
	...
	fileName := iClient.maeshReleaseVersion
	downloadURL := iClient.maeshReleaseDownloadURL
